...
namespace
{
   mrb_value rect_initialize(mrb_state* mrb, mrb_value self)
   {
      ...
      mrb_int args_c = mrb_get_args(mrb, "|ffff:", &x, &y, &width, &height, &kw_args);
      ...
      if (args_c > 0) {
         ...
         mrb_data_init(self, new Rect<float>(x, y, width, height), &r_rect2f_type);
      }
      else {
         for (int i = 0; i < kw_num; ++i) {
            if (mrb_undef_p(kw_values[i])) {
               mrb_data_init(self, new Rect<float>(), &r_rect2f_type);
               return self;
            }
         }
         
         float x = Ruby::to_float(mrb, kw_values[0]);
         float y = Ruby::to_float(mrb, kw_values[1]);
         float width = Ruby::to_float(mrb, kw_values[2]);
         float height = Ruby::to_float(mrb, kw_values[3]);
   ...
   {
      auto rect = static_cast<Rect<float>*>(mrb_data_get_ptr(mrb, self, &r_rect2f_type));
   ...
   {
      auto rect = static_cast<Rect<float>*>(mrb_data_get_ptr(mrb, self, &r_rect2f_type));
      auto output = String::format_object("#<Rect2f>", {
...
void ruby::bind_rect_float()
{
   auto* ref = Interpreter::bind_class("Rect2f");
